...
state_dtype = np.dtype([('local_occupancy_grid', np.uint8, (160, 160)), ('laser_scan',   np.float32, (580,)),])

Transition_dtype = np.dtype([('timestep', np.int32), ('state', state_dtype), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])

blank_ob = np.zeros(1, dtype=state_dtype)
blank_ob['local_occupancy_grid'] += 50
blank_trans = (0, blank_ob , 0, 0.0, False)
...
class ReplayMemory():
  ...
  def append(self, state, action, reward, terminal):
    global state_dtype
    new_state = np.squeeze(np.zeros(1, dtype = state_dtype), axis = 0)
    new_state['local_occupancy_grid'] = torch.tensor(state['local_occupancy_grid'][-1, :, :],dtype=torch.float32).mul(100).to(dtype=torch.uint8, device=torch.device('cpu'))
    new_state['laser_scan'] = torch.tensor(state['laser_scan'][-1, :],dtype=torch.float32).to(dtype=torch.float16, device=torch.device('cpu'))
    self.transitions.append((self.t, new_state, action, reward, not terminal), self.transitions.max)  # Store new transition with maximum priority
  ...
  def _get_transitions(self, idxs):
    ...
    batch_dim, _ = transitions_firsts.shape
    for b in range(batch_dim):
      for t in range(self.history - 2, -1, -1):  # e.g. 2 1 0
        if(transitions_firsts[b, t + 1]):
          transitions[b, t] = transitions[b, t + 1]
        transitions_firsts[b, t] = np.logical_or(transitions_firsts[b, t], transitions_firsts[b, t + 1])
      for t in range(self.history, self.history + self.n):  # e.g. 4 5 6
        if(t > self.history):
          transitions_firsts[b, t] = np.logical_or(transitions_firsts[b, t], transitions_firsts[b, t - 1])
        if(transitions_firsts[b, t]):
          transitions[b, t] = transitions[b, t - 1]
          transitions[b, t]['reward'] = 0.0
    return transitions
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    states = {'local_occupancy_grid': torch.tensor(np.copy(states['local_occupancy_grid']), device=self.device, dtype=torch.float32).div_(100),
    	       'laser_scan': torch.tensor(np.copy(states['laser_scan']), device=self.device, dtype=torch.float32)}
    next_states = all_states[:, self.n:self.n + self.history] 
    next_states = {'local_occupancy_grid': torch.tensor(np.copy(next_states['local_occupancy_grid']), device=self.device, dtype=torch.float32).div_(100),
    	       'laser_scan': torch.tensor(np.copy(next_states['laser_scan']), device=self.device, dtype=torch.float32)}   
